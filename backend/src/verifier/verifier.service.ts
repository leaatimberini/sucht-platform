...
export class VerifierService {
    ...
    async scanQr(qrId: string, user: User, eventId?: string): Promise<ScanResult> {
        ...
        try {
            ...
            if (productPurchase) {
                ...
                // Lógica de auto-validación de entrada (usa el evento del producto)
        ...
        try {
            ...
            if (userReward) {
                ...
                // --- AUTO-VALIDACIÓN DE ENTRADA AL CONSUMIR PREMIO ---
                // Usamos el eventId provisto por el contexto del escáner (si existe)
                if (eventId) {
                    await this.autoRedeemEntryTicket(userReward.user.id, eventId);
                } else {
                    this.logger.warn(`No se pudo auto-validar entrada para premio ${qrId} porque no se proveyó eventId de contexto.`);
                }
